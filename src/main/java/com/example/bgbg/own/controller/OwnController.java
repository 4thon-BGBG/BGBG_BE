...
public class OwnController {
    ...
    public ResponseEntity<?> getOwnById(
        ...
        return ResponseEntity
                .status(ResponseCode.SUCCESS_GET_OWN.getStatus().value())
                .body(new ResponseDTO<>(ResponseCode.SUCCESS_GET_OWN, own));
    }

    @Operation(summary = "전체 또는 카테고리 별 유저의 보유 품목 조회", description = "category 미입력 시 전체 조회 가능한 카테고리 별 보유 품목 조회 api")
    @GetMapping
    ...
    public ResponseEntity<?> updateOwn(
        ...
        return ResponseEntity
                .status(ResponseCode.SUCCESS_UPDATE_OWN.getStatus().value())
    ...
    public ResponseEntity<?> deleteOwn(
        ...
        return ResponseEntity
                .status(ResponseCode.SUCCESS_DELETE_OWN.getStatus().value())
